...
import random
from base.utils import make_parts_shape, make_shape_parts
...
class DeepfashionPoseDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        pose = np.concatenate((pose, mask), axis=0)
        ...
        # t_pose = self.make_joint_img(self.img_shape, self.joint_order, self.index["joints"][t_idx])
        ...
        t_pose = np.concatenate((t_pose, t_mask), axis=0)
...
if __name__ == '__main__':
    ...
    dataset = DeepfashionPoseDataset((128, 128, 3), base_dir, index_dir, map_dir, training=False)
    dataloader = DataLoader(dataset, batch_size=1, shuffle=True, num_workers=1)
    for index, sample in enumerate(dataloader):
        imgs = sample['input']['s_map'].numpy()
        poses = sample['input']['pose'].numpy()
        tposes = sample['input']['t_pose'].numpy()
        target = sample['input']['t_map'].numpy()
